...
//	}
	...
	protected  Object[] RegistQueryModel(String name,Class<?> autoModelBaseType,DatasourceTypeEnum dsType)
	{
		...
		{
			///输出日志 重复
			Log.d("", String.format( "CFLog: AndroidAppCF.SuncereAutoActivity_%s_重复插件名%",this.getClass().getSimpleName(),name));
			return null;
		}
		if(autoModelBaseType==null)
		{
			Log.d("", String.format( "CFLog: AndroidAppCF.SuncereAutoActivity_%s_缺少模型类型 autoModelBaseType",this.getClass().getSimpleName()));
			return null;
		}
		AutoBaseModel model=(AutoBaseModel) TypeHelper.NewIntance(autoModelBaseType);
		if(model==null)
		{
			Log.d("", String.format( "CFLog: AndroidAppCF.SuncereAutoActivity_%s_类型 %s 实例化失败，请定义无参构造函数",this.getClass().getSimpleName(),autoModelBaseType.getName()));
			return null;
		}
